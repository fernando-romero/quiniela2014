...
updateBonusPoints = function() {
	ref.child('bonus').once('value', function(bonusesSnap) {
		bonusesSnap.forEach(function(bonusSnap) {
			var num = parseInt(matchSnap.name(), 10);
			var team = matchSnap.val().team;

			bonusSnap.child('forecasts').forEach(function(forecastSnap){
				var points = 0;
				var name = forecastSnap.name();
				var f_team = forecastSnap.val().team;
				if (team == f_team) points += 20;

				set('bonus/' + num + '/forecasts/' + name + '/points', points);
				set('players/' + name + '/bonus/' + num + '/points', points);
			});
